...
datdir = os.path.join(prjdir, "derivatives/fix_agg_blankcomps/")
outdir = os.path.join(prjdir, "derivatives/nilearn_univglm_blankcomps_psc/")
...
math_img("beta_img * (zmap_img[:,:,:,np.newaxis] > 3.29)",
         beta_img = os.path.join(group_outdir, 'p4p1', 'group_p4p1_beta.nii.gz'),
         zmap_img = os.path.join(group_outdir, 'p4p1', 'group_p4p1_zmap.nii.gz')
         ).to_filename(
    os.path.join(group_outdir, 'p4p1', 'group_p4p1_beta_th001.nii.gz'))
secondlvlglm_p4p1.generate_report('constant').save_as_html(
    os.path.join(group_outdir, 'p4p1', 'report.html'))
